...
! function($) {
    ...
    $.flexslider = function(el, options) {
        var watchedEventClearTimer, slider = $(el),
        ...
        $.data(el, "flexslider", slider), methods = {
            init: function() {
                slider.animating = !1, slider.currentSlide = parseInt(slider.vars.startAt || 0, 10), isNaN(slider.currentSlide) && (slider.currentSlide = 0), slider.animatingTo = slider.currentSlide, slider.atEnd = 0 === slider.currentSlide || slider.currentSlide === slider.last, slider.containerSelector = slider.vars.selector.substr(0, slider.vars.selector.search(" ")), slider.slides = $(slider.vars.selector, slider), slider.container = $(slider.containerSelector, slider), slider.count = slider.slides.length, slider.syncExists = 0 < $(slider.vars.sync).length, "slide" === slider.vars.animation && (slider.vars.animation = "swing"), slider.prop = vertical ? "top" : slider.vars.rtl ? "marginRight" : "marginLeft", slider.args = {}, slider.manualPause = !1, slider.stopped = !1, slider.started = !1, slider.startTimeout = null, slider.transitions = !slider.vars.video && !fade && slider.vars.useCSS && function() {
                    var i, obj = document.createElement("div"),
                    ...
                    return !1
                }(), slider.isFirefox = -1 < navigator.userAgent.toLowerCase().indexOf("firefox"), (slider.ensureAnimationEnd = "") !== slider.vars.controlsContainer && (slider.controlsContainer = 0 < $(slider.vars.controlsContainer).length && $(slider.vars.controlsContainer)), "" !== slider.vars.manualControls && (slider.manualControls = 0 < $(slider.vars.manualControls).length && $(slider.vars.manualControls)), "" !== slider.vars.customDirectionNav && (slider.customDirectionNav = 2 === $(slider.vars.customDirectionNav).length && $(slider.vars.customDirectionNav)), slider.vars.randomize && (slider.slides.sort(function() {
                    return Math.round(Math.random()) - .5
                }), slider.container.empty().append(slider.slides)), slider.doMath(), slider.setup("init"), slider.vars.controlNav && methods.controlNav.setup(), slider.vars.directionNav && methods.directionNav.setup(), slider.vars.keyboard && (1 === $(slider.containerSelector).length || slider.vars.multipleKeyboard) && $(document).on("keyup", function(event) {
                    var event = event.keyCode;
                    slider.animating || 39 !== event && 37 !== event || (event = slider.vars.rtl ? 37 === event ? slider.getTarget("next") : 39 === event && slider.getTarget("prev") : 39 === event ? slider.getTarget("next") : 37 === event && slider.getTarget("prev"), slider.flexAnimate(event, slider.vars.pauseOnAction))
                ...
                }), slider.vars.pauseInvisible && methods.pauseInvisible.isHidden() || (0 < slider.vars.initDelay ? slider.startTimeout = setTimeout(slider.play, slider.vars.initDelay) : slider.play())), asNav && methods.asNav.setup(), touch && slider.vars.touch && methods.touch(), fade && !slider.vars.smoothHeight || $(window).on("resize orientationchange focus", methods.resize), slider.find("img").attr("draggable", "false"), setTimeout(function() {
                    slider.vars.start(slider)
                }, 200)
            },
            asNav: {
                setup: function() {
                    slider.asNav = !0, slider.animatingTo = Math.floor(slider.currentSlide / slider.move), slider.currentItem = slider.currentSlide, slider.slides.removeClass(namespace + "active-slide").eq(slider.currentItem).addClass(namespace + "active-slide"), msGesture ? (el._slider = slider).slides.each(function() {
                        this._gesture = new MSGesture, (this._gesture.target = this).addEventListener("MSPointerDown", function(e) {
                            ...
                            var e = $(this),
                                target = e.index();
                            $(slider.vars.asNavFor).data("flexslider").animating || e.hasClass("active") || (slider.direction = slider.currentItem < target ? "next" : "prev", slider.flexAnimate(target, slider.vars.pauseOnAction, !1, !0, !0))
                        })
                        ...
                        posFromX <= 0 && e.hasClass(namespace + "active-slide") ? slider.flexAnimate(slider.getTarget("prev"), !0) : $(slider.vars.asNavFor).data("flexslider").animating || e.hasClass(namespace + "active-slide") || (slider.direction = slider.currentItem < target ? "next" : "prev", slider.flexAnimate(target, slider.vars.pauseOnAction, !1, !0, !0))
                    })
                }
            },
            controlNav: {
                ...
                setupPaging: function() {
                    var item, type = "thumbnails" === slider.vars.controlNav ? "control-thumbs" : "control-paging",
                        j = 1;
                    if (slider.controlNavScaffold = $('<ol class="' + namespace + "control-nav " + namespace + type + '"></ol>'), 1 < slider.pagingCount)
                        for (var i = 0; i < slider.pagingCount; i++) {
                            void 0 === (slide = slider.slides.eq(i)).attr("data-thumb-alt") && slide.attr("data-thumb-alt", ""), item = $("<a></a>").attr("href", "#").text(j), "thumbnails" === slider.vars.controlNav && (item = $("<img/>").attr("src", slide.attr("data-thumb"))), "" !== slide.attr("data-thumb-alt") && item.attr("alt", slide.attr("data-thumb-alt")), "thumbnails" === slider.vars.controlNav && !0 === slider.vars.thumbCaptions && "" !== (slide = slide.attr("data-thumbcaption")) && void 0 !== slide && (slide = $("<span></span>").addClass(namespace + "caption").text(slide), item.append(slide));
                            var slide = $("<li>");
                            item.appendTo(slide), slide.append("</li>"), slider.controlNavScaffold.append(slide), j++
                        }(slider.controlsContainer ? $(slider.controlsContainer) : slider).append(slider.controlNavScaffold), methods.controlNav.set(), methods.controlNav.active(), slider.controlNavScaffold.on(eventType, "a, img", function(event) {
                            var $this, target;
                            event.preventDefault(), "" !== watchedEvent && watchedEvent !== event.type || ($this = $(this), target = slider.controlNav.index($this), $this.hasClass(namespace + "active")) || (slider.direction = target > slider.currentSlide ? "next" : "prev", slider.flexAnimate(target, slider.vars.pauseOnAction)), "" === watchedEvent && (watchedEvent = event.type), methods.setToClearWatchedEvent()
                        })
                },
                setupManual: function() {
                    slider.controlNav = slider.manualControls, methods.controlNav.active(), slider.controlNav.on(eventType, function(event) {
                        var $this, target;
                        event.preventDefault(), "" !== watchedEvent && watchedEvent !== event.type || ($this = $(this), target = slider.controlNav.index($this), $this.hasClass(namespace + "active")) || (target > slider.currentSlide ? slider.direction = "next" : slider.direction = "prev", slider.flexAnimate(target, slider.vars.pauseOnAction)), "" === watchedEvent && (watchedEvent = event.type), methods.setToClearWatchedEvent()
                    })
                },
                set: function() {
            ...
            directionNav: {
                setup: function() {
                    ...
                    slider.customDirectionNav ? slider.directionNav = slider.customDirectionNav : slider.controlsContainer ? ($(slider.controlsContainer).append(directionNavScaffold), slider.directionNav = $("." + namespace + "direction-nav li a", slider.controlsContainer)) : (slider.append(directionNavScaffold), slider.directionNav = $("." + namespace + "direction-nav li a", slider)), methods.directionNav.update(), slider.directionNav.on(eventType, function(event) {
                        var target;
                        event.preventDefault(), "" !== watchedEvent && watchedEvent !== event.type || (target = $(this).hasClass(namespace + "next") ? slider.getTarget("next") : slider.getTarget("prev"), slider.flexAnimate(target, slider.vars.pauseOnAction)), "" === watchedEvent && (watchedEvent = event.type), methods.setToClearWatchedEvent()
                    })
                },
                update: function() {
            ...
            touch: function() {
                var startX, startY, offset, cwidth, dx, startT, onTouchMove, onTouchEnd, scrolling = !1,
                    localX = 0,
                    localY = 0,
                    accDx = 0;
                msGesture ? (el.style.msTouchAction = "none", el._gesture = new MSGesture, (el._gesture.target = el).addEventListener("MSPointerDown", function(e) {
                ...
                }, !1), el._slider = slider, el.addEventListener("MSGestureChange", function(e) {
                    e.stopPropagation();
                    var transX, transY, slider = e.target._slider;
                    slider && (transX = -e.translationX, transY = -e.translationY, accDx += vertical ? transY : transX, dx = (slider.vars.rtl ? -1 : 1) * accDx, scrolling = vertical ? Math.abs(accDx) < Math.abs(-transX) : Math.abs(accDx) < Math.abs(-transY), e.detail === e.MSGESTURE_FLAG_INERTIA ? setImmediate(function() {
                        el._gesture.stop()
                    }) : (!scrolling || 500 < Number(new Date) - startT) && (e.preventDefault(), !fade) && slider.transitions && (slider.vars.animationLoop || (dx = accDx / (0 === slider.currentSlide && accDx < 0 || slider.currentSlide === slider.last && 0 < accDx ? Math.abs(accDx) / cwidth + 2 : 1)), slider.setProps(offset + dx, "setTouch")))
                }, !1), el.addEventListener("MSGestureEnd", function(e) {
                    e.stopPropagation();
                    var updateDx, target, e = e.target._slider;
                    e && (e.animatingTo !== e.currentSlide || scrolling || null === dx || (target = 0 < (updateDx = reverse ? -dx : dx) ? e.getTarget("next") : e.getTarget("prev"), e.canAdvance(target) && (Number(new Date) - startT < 550 && 50 < Math.abs(updateDx) || Math.abs(updateDx) > cwidth / 2) ? e.flexAnimate(target, e.vars.pauseOnAction) : fade || e.flexAnimate(e.currentSlide, e.vars.pauseOnAction, !0)), offset = dx = startY = startX = null, accDx = 0)
                    ...
                    var updateDx, target;
                    el.removeEventListener("touchmove", onTouchMove, !1), slider.animatingTo !== slider.currentSlide || scrolling || null === dx || (target = 0 < (updateDx = reverse ? -dx : dx) ? slider.getTarget("next") : slider.getTarget("prev"), slider.canAdvance(target) && (Number(new Date) - startT < 550 && 50 < Math.abs(updateDx) || Math.abs(updateDx) > cwidth / 2) ? slider.flexAnimate(target, slider.vars.pauseOnAction) : fade || slider.flexAnimate(slider.currentSlide, slider.vars.pauseOnAction, !0)), el.removeEventListener("touchend", onTouchEnd, !1), offset = dx = startY = startX = null
            ...
            smoothHeight: function(dur) {
                var $obj;
                vertical && !fade || ($obj = fade ? slider : slider.viewport, dur ? $obj.animate({
            ...
            pauseInvisible: {
                ...
                init: function() {
                    ...
                    visProp && (visProp = visProp.replace(/[H|h]idden/, "") + "visibilitychange", document.addEventListener(visProp, function() {
            ...
            fade ? (slider.vars.rtl ? slider.slides.css({
                ...
                opacity: 1
            }, slider.vars.animationSpeed, slider.vars.easing)), slider.vars.smoothHeight && methods.smoothHeight()) : ("init" === type && (slider.viewport = $('<div class="' + namespace + 'viewport"></div>').css({
                overflow: "hidden",
                position: "relative"
            }).appendTo(slider).append(slider.container), slider.cloneCount = 0, slider.cloneOffset = 0, reverse) && (arr = $.makeArray(slider.slides).reverse(), slider.slides = $(arr), slider.container.empty().append(slider.slides)), slider.vars.animationLoop && !carousel && (slider.cloneCount = 2, slider.cloneOffset = 1, "init" !== type && slider.container.find(".clone").remove(), slider.container.append(methods.uniqueID(slider.slides.first().clone().addClass("clone")).attr("aria-hidden", "true")).prepend(methods.uniqueID(slider.slides.last().clone().addClass("clone")).attr("aria-hidden", "true"))), slider.newSlides = $(slider.vars.selector, slider), sliderOffset = reverse ? slider.count - 1 - slider.currentSlide + slider.cloneOffset : slider.currentSlide + slider.cloneOffset, vertical && !carousel ? (slider.container.height(200 * (slider.count + slider.cloneCount) + "%").css("position", "absolute").width("100%"), setTimeout(function() {
                ...
                slider.doMath(), slider.vars.rtl ? slider.newSlides.css({
            ...
            }, "init" === type ? 100 : 0))), carousel || slider.slides.removeClass(namespace + "active-slide").eq(slider.currentSlide).addClass(namespace + "active-slide"), slider.vars.init(slider)
        }, slider.doMath = function() {
            var slide = slider.slides.first(),
                slideMargin = slider.vars.itemMargin,
                minItems = slider.vars.minItems,
                maxItems = slider.vars.maxItems;
            slider.w = (void 0 === slider.viewport ? slider : slider.viewport).width(), slider.isFirefox && (slider.w = slider.width()), slider.h = slide.height(), slider.boxPadding = slide.outerWidth() - slide.width(), carousel ? (slider.itemT = slider.vars.itemWidth + slideMargin, slider.itemM = slideMargin, slider.minW = minItems ? minItems * slider.itemT : slider.w, slider.maxW = maxItems ? maxItems * slider.itemT - slideMargin : slider.w, slider.itemW = slider.minW > slider.w ? (slider.w - slideMargin * (minItems - 1)) / minItems : slider.maxW < slider.w ? (slider.w - slideMargin * (maxItems - 1)) / maxItems : slider.vars.itemWidth > slider.w ? slider.w : slider.vars.itemWidth, slider.visible = Math.floor(slider.w / slider.itemW), slider.move = 0 < slider.vars.move && slider.vars.move < slider.visible ? slider.vars.move : slider.visible, slider.pagingCount = Math.ceil((slider.count - slider.visible) / slider.move + 1), slider.last = slider.pagingCount - 1, slider.limit = 1 === slider.pagingCount ? 0 : slider.vars.itemWidth > slider.w ? slider.itemW * (slider.count - 1) + slideMargin * (slider.count - 1) : (slider.itemW + slideMargin) * slider.count - slider.w - slideMargin) : (slider.itemW = slider.w, slider.itemM = slideMargin, slider.pagingCount = slider.count, slider.last = slider.count - 1), slider.computedW = slider.itemW - slider.boxPadding, slider.computedM = slider.itemM
        }, slider.update = function(pos, action) {
            slider.doMath(), carousel || (pos < slider.currentSlide ? slider.currentSlide += 1 : pos <= slider.currentSlide && 0 !== pos && --slider.currentSlide, slider.animatingTo = slider.currentSlide), slider.vars.controlNav && !slider.manualControls && ("add" === action && !carousel || slider.pagingCount > slider.controlNav.length ? methods.controlNav.update("add") : ("remove" === action && !carousel || slider.pagingCount < slider.controlNav.length) && (carousel && slider.currentSlide > slider.last && (--slider.currentSlide, --slider.animatingTo), methods.controlNav.update("remove", slider.last))), slider.vars.directionNav && methods.directionNav.update()
            ...
            --slider.count, slider.last = slider.count - 1, (isNaN(obj) ? $(obj, slider.slides) : vertical && reverse ? slider.slides.eq(slider.last) : slider.slides.eq(obj)).remove(), slider.doMath(), slider.update(pos, "remove"), slider.slides = $(slider.vars.selector + ":not(.clone)", slider), slider.setup(), slider.vars.removed(slider)
        }, methods.init()
        ...
        if ("object" == typeof(options = void 0 === options ? {} : options)) return this.each(function() {
            var $this = $(this),
